"""Ecrit Lancer_Workflows_Q_Builder.bat en CRLF.

Passer par un script plutot que par un editeur garantit les fins de ligne
Windows : un .bat en LF peut deraper sur `goto`, et l'archive zip de GitHub
ignore l'attribut eol du .gitattributes.
"""
...
def main():
    target = HERE / "Lancer_Workflows_Q_Builder.bat"
    target.write_text(BAT.replace("\r\n", "\n").replace("\n", "\r\n"), encoding="ascii")
    data = target.read_bytes()
    crlf = data.count(b"\r\n")
    lf = data.count(b"\n")
    print(f"{target.name} ecrit : {crlf} CRLF sur {lf} fins de ligne")
    if crlf != lf:
        raise SystemExit("fins de ligne incoherentes")
